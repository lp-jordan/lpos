...
### Recording
The record button lives under different command IDs per body: **cinema bodies (FX6)** take `CrCommandId_MovieRecButtonToggle`; **Alpha-based bodies (FX3)** reject that with `NotSupported (0x8003)` and take `CrCommandId_MovieRecord`. `preferredRecCommand` is **seeded from the model** in the `CameraSession` constructor (FX3 → `MovieRecord`, else `MovieRecButtonToggle`) so the *first* toggle is already correct — a wasted probe press adds ~400ms and would desync an FX3 from the FX6s in a synchronized REC roll. `sendRecToggleLocked()` tries the seeded command first and still falls back to the other on `NotSupported` (re-caching), so a mismodelled body self-corrects, but a correctly-modelled one never pays the fallback. Both are sent as a full Down-then-Up press (a lone Down does not register), and each full press toggles the state once. Because it only *toggles*, `startRecording` / `stopRecording` first read the real state and skip the press if already in the target state. If a body accepts neither command, the error says so.
