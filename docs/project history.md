...
## 2026-06-28T12:30:00Z — Cold storage: true all-version purge deletes + live Backblaze footprint reconciliation

**User prompt:** "I can change the bucket setting to Keep only the last version of the file but yes, ideally LPOS would be able to make meaningful delete decisions so we're not ever needing to go back into backblaze. LPOS is the monitor and the gun when necessary. But our system prevents accidental deletes."

**Decisions (confirmed via question):** (1) LPOS purges all versions itself (self-sufficient, not dependent on the bucket lifecycle rule). (2) Build the monitor + the gun together this pass (deferred the bucket-wide one-time dark-mass cleanup action).

**Assistant response summary:** Implemented two capabilities on the existing cold-storage admin, leaving the accidental-delete safety flow (missing → retainDays → explicit admin Approve/Spare, empty-prefix refusal, admin DM on queue) untouched. **The gun:** rewrote `deleteOne`/`deletePrefix` in `b2-cold-storage-browser.ts` to do a TRUE purge — a shared `purgeVersions()` helper paginates `ListObjectVersions` over the key/prefix and hard-deletes every version AND hide-marker in batches of 1000 via `DeleteObjects`, reclaiming space immediately regardless of bucket lifecycle config. (Previously a versionless `DeleteObject` only wrote a hide-marker, leaving data billing — the root cause of the 819-vs-5000 GB gap diagnosed earlier.) approve-all and the bucket browser inherit the new behavior since they call `deleteOne`/`deletePrefix`. **The monitor:** new `reconcileBucketFootprint()` walks every version in the bucket and tallies current-version bytes vs non-current/hidden (old-version) bytes vs hide-markers vs total billed; exposed at `GET /api/admin/cold-storage/reconcile` (admin-only, on-demand — not on the 60s poll since a full-version walk can be slow). UI: added a "Live Backblaze footprint" panel to `ColdStorageSection.tsx` with a "Check live footprint" button that shows the breakdown next to LPOS's tracked total (current should match) and flags reclaimable old-version mass; updated the per-object and approve-all delete confirmations to say "permanently purge all versions / reclaims space immediately."

**Files changed:**
- `lpos-dashboard/lib/services/b2-cold-storage-browser.ts` — purgeVersions helper; deleteOne/deletePrefix now purge all versions; reconcileBucketFootprint(); swapped imports (ListObjectVersionsCommand + DeleteObjectsCommand in, DeleteObjectCommand out).
- `lpos-dashboard/app/api/admin/cold-storage/reconcile/route.ts` — new admin-only GET endpoint.
- `lpos-dashboard/components/settings/ColdStorageSection.tsx` — footprint panel + state + fetch; purge-aware confirm copy.
- `docs/README.md` — Raw Footage Cold Storage section: documented version-purge deletes, reconciliation, manual-approval gate; added reconcile route + updated key-files table.
- `docs/project history.md`, `docs/changelog.json` — this entry.

**Decision rationale:** `ListObjectVersions` + versioned `DeleteObjects` is the only S3-API way to (a) see and (b) reclaim non-current versions on a versioned B2 bucket; `ListObjectsV2` (used elsewhere) is blind to them. Wiring the purge behind the *existing* `deleteOne`/`deletePrefix` signatures meant approve-all, per-object delete, and the browser all became "real" with zero new gating, preserving the safety model. Reconciliation kept on-demand (button) rather than on the auto-poll to avoid a full-version walk every 60s on a large bucket.

**Alternatives considered:** Hide-and-rely-on-bucket-lifecycle (rejected by user — wanted LPOS self-sufficient). Batch `DeleteObjects` chosen over per-object `DeleteObject` loops for throughput on large version sets.

**Commands/checks run:** `npx tsc --noEmit` — 0 errors. No dev server started (per workspace rule). No live B2 calls made from this session.

**Assumptions / follow-ups:** User will set the bucket lifecycle to "Keep only the last version" to bulk-reclaim the pre-existing ~4,200 GB of old versions (the in-app purge only reclaims going forward, per-object as files are retired). Deferred follow-up: a gated one-time "Reclaim old versions" maintenance action that purges only non-current versions bucket-wide without touching live files. The footprint walk is unbounded — fine for current bucket size; revisit if it ever times out the request.

---

## 2026-06-30T21:05:00Z — Internal Review: layout resolved to "scoped media page"

**User prompt:** "I'm imagining the UI will basically look like the media page - a card or list view of the selected videos (the bundle) the user can review one at a time in the sidebar (or theater mode). We're basically just giving them a dedicated environment to watch the isolated group of videos in the same way they would in the media page. Make sense?"

**Assistant response summary:** Confirmed — the review page is the media page scoped to the bundle, not a bespoke reel. Identified `components/projects/MediaTab.tsx` as the component that renders the media list/card view + `MediaDetailPanel` sidebar + `VideoTheaterMode`, so the build is "render `MediaTab` filtered to the bundle's asset set, themed near-black/gold." Updated `docs/internal-review-spec.md`: replaced the prior left-rail-reel recommendation with reuse of `MediaTab` (parameterized to accept an explicit asset list/filter rather than always loading the full project); updated the files-touched table and resolved the previously-open layout question.

**Files changed:**
- `docs/internal-review-spec.md` — layout section reworked to "scoped media page"; `MediaTab` added to files-touched as the parameterization point; `MediaDetailPanel`/`VideoTheaterMode` noted as reused unchanged.
- `docs/project history.md`, `docs/changelog.json` — this log.

**Decision rationale:** Reusing `MediaTab` scoped to the bundle removes the last net-new UI piece (no custom reel/navigation); the asset set is the only thing that differs from the normal media page. Parameterize rather than fork `MediaTab` to avoid a near-duplicate that drifts.

**Commands run:** grep to confirm the media list/detail/theater components (`MediaTab`, `MediaDetailPanel`, `VideoTheaterMode`). No code/tests/builds.

**Assumptions / follow-ups:** Spec now has no open design questions — only the implementation detail of how to scope `MediaTab` to an explicit asset set. No production code touched.

---

## 2026-06-30T21:40:00Z — Studio Lighting: master All On/Off toggle next to Presets

**User prompt:** "Can you put a little "All On/Off" toggle next to presets in the lighting page of studio in lpos? This should disable and enable all lights."

**Assistant response summary:** Added a master power toggle beside the Presets trigger on the studio lighting page (`/slate`). New `AllPowerToggle` switch component in `LightingPresets.tsx` (track + thumb + "All On"/"All Off" label, green when on, "Working…" while busy). `LightingPanel` wraps the existing `PresetsTrigger` and the new toggle in a `lp-lighting-actions-row` flex row, computes `allOn` (true only when every fixture's `state.power === true`), and on tap runs `handleAllPower(on)` which loops all fixtures issuing `sendCommand('setPower', nodeId, { on })`. The loop is sequential (awaited per fixture) because the Amaran service keys its pending WebSocket requests by action name only, so concurrent `set_sleep` calls would collide. Toggle is disabled when disconnected or no fixtures; `bulkBusy` guards re-entrancy. Added matching CSS.

**Files changed:**
- `lpos-dashboard/components/slate/LightingPresets.tsx` — new exported `AllPowerToggle` component + props interface.
- `lpos-dashboard/components/slate/LightingPanel.tsx` — import `AllPowerToggle`; `bulkBusy` state + `handleAllPower`; `allOn` derivation; render trigger+toggle in a `lp-lighting-actions-row`.
- `lpos-dashboard/app/globals.css` — `.lp-lighting-actions-row` + `.lp-all-power-toggle` / track / thumb / label styles.
- `docs/project history.md`, `docs/changelog.json` — this entry.

**Decision rationale:** Reused the existing per-fixture `sendCommand('setPower', …)` path rather than adding a new bulk API method/route — no server change needed, and it inherits the service's state-emit + socket broadcast so the UI and individual ON/OFF buttons stay in sync. Sequential dispatch is required by the service's action-keyed pending map (same constraint preset-apply already follows). Toggle reflects `allOn` (thumb-right only when truly all on) so the label is always honest; from a mixed state a tap turns everything on.

**Alternatives considered:** A new `/api/studio/lighting` bulk `setAllPower` method that loops server-side (rejected — more surface area for no real gain; the client loop reuses existing emit/broadcast). `anyOn` semantics for the thumb (rejected — would show "All On" while only some are lit).
...
## 2026-07-09T14:20:49Z — LPOS: force reset now prunes stale Cloudflare videos (collapse to the live one)

**User prompt:** "i need to be able to forcefully delete all existing versions of a video in cloudflare except for the most recent - ensuring that most recent is actually live in cf" (clarified: the *current force reset function* should stop preserving old Cloudflare versions of that video; chosen end-state = "collapse to the live one").

**Response summary:** The per-asset force reset (`DELETE /api/projects/[projectId]/media/[assetId]/leaderpass`) previously only blanked LPOS's local `cloudflare` pointer, leaving the actual CF video(s) alive in the account as orphans until the once-daily reconciler noticed. It now forcefully collapses the asset's Cloudflare footprint to a single confirmed-live video: it enumerates every CF video for the asset, keeps the most-recent one that is actually `readyToStream` in Cloudflare, deletes all the others, and repoints LPOS at the survivor. A safety rail refuses to delete anything when no candidate is confirmed live.

**Files changed:**
- `lib/services/cloudflare-publish.ts` — new exported `pruneCloudflareVersionsForAsset(assetId, { preferUid })` + `CloudflarePruneResult`; imports `getCloudflareVideoState`, `listCloudflareVideos`, `listCloudflareUidsForAsset`, `markOrphanPurged`.
- `lib/store/canonical-asset-store.ts` — new exported `listCloudflareUidsForAsset(assetId)` (all CF `provider_asset_id` for the asset across versions/attempts, newest-first).
- `app/api/projects/[projectId]/media/[assetId]/leaderpass/route.ts` — DELETE handler runs the prune first; on a live survivor it repoints local state at it instead of wiping; with no live survivor it falls back to the original wipe-to-`none`.
- `docs/README.md` — documented the force reset / version prune under the LeaderPass / Cloudflare Stream Pipeline section.

**Implementation summary:** Enumeration keys off Cloudflare's `creator` field (set to `assetId.slice(0,64)` as `Upload-Creator` on upload), unioned with `distribution_records` UIDs and the caller's `preferUid`, so videos whose DB link was lost are still caught. Each candidate's live status is probed via `getCloudflareVideoState` (`readyToStream`); a 404 is treated as already-gone, any other probe error as "still exists, not live" (neither deleted nor eligible as survivor). Survivor = the `preferUid` if it is itself live, else the newest live candidate by `created`. All other still-existing candidates are `deleteCloudflareVideo`'d; failures are recorded as `cloudflare_orphans` (reason `delete_failed`) and successes clear any existing orphan row via `markOrphanPurged`. The route repoints via `patchAsset({ cloudflare: { uid: survivor, status: 'ready', ... } })`, which (status='ready' ⇒ not a new attempt) updates the latest CF `distribution_record` in place, so `getLiveCloudflareUids()` follows the survivor.

**Decision rationale:** The user chose "collapse to the live one" over "wipe all + re-push fresh". The `readyToStream` gate before any delete directly satisfies "ensuring that most recent is actually live in CF" and guarantees the operation never leaves an asset with zero playable video. Wiring into the existing reset DELETE (rather than a new endpoint/button) matches the user's framing that the *current* force reset should stop preserving old versions. Reused the existing orphan bookkeeping so failed deletes still funnel to the 24 h reconciler.

**Alternatives considered:** New dedicated admin endpoint/button and a whole-library sweep (the user's initial scope questions) — both declined in favor of fixing the existing force reset. Deleting all CF videos and forcing a re-push (Design B) — rejected by the user in favor of keeping the live survivor. Rewriting stale `distribution_records` rows for deleted UIDs — deliberately out of scope; only the latest CF record is reconciled (via the survivor repoint), matching the existing publish pipeline's ownership of those rows.

**Commands/checks run:** `tsc --noEmit -p tsconfig.json` → 0 errors. Not runtime-verified (dev server is user-managed): the live-survivor prune/delete against the real Cloudflare account and the no-live-survivor fallback still need a manual pass.

**Assumptions / follow-ups:** Assumes `creator` was stamped on uploads (true since the current upload path). `pruneCloudflareVersionsForAsset` pages the entire CF account per call (reuses `listCloudflareVideos`) — acceptable for a manual reset, could later use Cloudflare's `?creator=` filter. Follow-up: the "Reset & re-push" button label is now slightly inaccurate for the live-survivor case (it collapses rather than wipes) — revisit UI copy if desired; and consider surfacing the prune summary (kept/deleted counts) in the panel.

## 2026-07-09T14:44:11Z — LPOS: fix Cloudflare version selection (per-version attempt_number compared across versions)

**User prompt:** After the force-reset prune shipped, the reporting user found an asset (ToddMcIntyre_Promo, current v3) that "still shows as stale in LPOS and force reset seems stuck," despite the prune log correctly keeping the live v3 UID (`8246ddc15efa`) and deleting two stale v2 UIDs.

**Response summary:** Root cause was a latent selection bug, not the prune. Both `pickLatestDistributionAnyVersion` (drives `cloudflare` projection + `isStale`) and `getLiveCloudflareUids` (drives the admin delete guard, orphan reconciler, and the new prune) ranked Cloudflare distribution records by `attempt_number`, which is a **per-version** counter. This asset's v2 had 3 push attempts while v3 had 2, so the selectors picked v2's attempt-3 record — a video already deleted from Cloudflare — over v3's attempt-2 record (the actually-live `8246`). Result: the asset read as stale, played a dead UID, and `getLiveCloudflareUids` protected the dead UID while leaving the real live one unguarded. Fixed both selectors to rank by `(version_number, attempt_number)` — newest version's latest attempt — so a live current-version publication always wins. The prune itself worked correctly; the DB was already consistent (v3 → ready `8246`) after the reset, so the "stuck uploading" was a stale pre-reset screenshot compounded by the stale-flag bug.

**Files changed:**
- `lib/store/canonical-asset-store.ts` — `pickLatestDistributionAnyVersion` now iterates `bundle.versions` (version_number DESC) and returns the newest version's latest attempt for the provider, instead of the global `bundle.distributions` head (which was ordered attempt_number DESC across versions). `getLiveCloudflareUids` SQL now selects the max `version_number` (among CF records with a non-null uid) per asset, then the max `attempt_number` within that version, instead of the global max `attempt_number`.

**Implementation summary:** `attempt_number` resets its meaning per (version, provider) because `createOrUpdateDistributionRecord` computes a new attempt as `(latest-for-this-version.attempt_number) + 1`. Comparing it across versions is therefore invalid. The projection fix reuses the existing per-version `pickLatestDistribution`. The live-UID fix adds a two-level correlated subquery (max version, then max attempt within it), both filtered to non-null `provider_asset_id`. Verified against the production `lpos-canonical-assets.sqlite`: the corrected live-UID query returns `8246…` (v3) for the reported asset, and a scan found **24 active assets** whose live/staleness selection was previously wrong (older version re-pushed more times than the current version) — all now corrected.

**Decision rationale:** Rank by version first, attempt second — this is what "the most recent Cloudflare publication" actually means and restores the documented intent ("an unstaged v2 still surfaces that v1 is live") without the cross-version attempt confusion. Chose to fix the selectors rather than delete the leftover stale distribution_records, because the selectors are the correct authority and the historical rows are harmless once ignored. No prune/route changes were needed — the earlier commit behaved correctly.

**Alternatives considered:** Reconciling/deleting stale distribution_records for dead UIDs during the prune (rejected — treats the symptom, and the publish pipeline owns those rows). Ordering `bundle.distributions` by `(version_number, attempt_number)` in the bundle query itself (rejected — other providers rely on the current ordering; a targeted change to the two cloudflare selectors is lower-risk).

**Commands/checks run:** `tsc --noEmit -p tsconfig.json` → 0 errors. Direct `sqlite3` validation of both corrected queries against production data (live-UID query returns v3 `8246`; 24 assets were mis-selected before). Not runtime-verified in the running app (dev server is user-managed) — the reporting user should refresh the media panel to confirm the asset now reads ready/not-stale.

**Assumptions / follow-ups:** Assumes version ranking is the intended "latest publication" semantics (matches the isStale intent). Follow-up: the reporting user should hard-refresh; the other 23 affected assets will self-correct on next read. Optional later cleanup: prune could tombstone dead distribution_records, and the live-survivor repoint could avoid stamping an older-version survivor onto the current version's record (only relevant when the current version never became live — not this case).

## 2026-07-09T17:13:45Z — LPOS: fix Firefox CF Stream playback (backfill missing tailnet allowedOrigin on 214 legacy videos)

**User prompt:** "Firefox is having playback issues in LPOS again." followed by a full Firefox console dump for the Todd McIntyre project media page. Later steer: "I think the answer is just that we didn't initially have the tailnet set to auto include on upload."

**Response summary:** The only real error in the console (amid healthy thumbnail/websocket/RSC traffic) was a CORS block on the Cloudflare Stream HLS manifest: `Access-Control-Allow-Origin does not match 'https://videodelivery.net'` for `customer-ze85nctblsmzqbrg.cloudflarestream.com/9c43a186…/manifest/video.m3u8`. Queried the Cloudflare API and found the video (C3.mp4, Todd McIntyre project) had `allowedOrigins = ['app.leaderpass.com']` — missing the LPOS tailnet host `lpos.tail856ed3.ts.net`. The `videodelivery.net` in the error is Cloudflare's sentinel fallback, not a configured origin. A full-account scan (538 videos) found **214 videos locked to leaderpass-only** (206 of them `ready`), i.e. ~40% of the library was broken for Firefox in-app playback. Firefox-specific because hls.js/MSE fetches the manifest via a CORS-checked XHR, whereas Safari/native HLS skips the check. Git history confirmed the user's theory: commit `555c779` (2026-06-28) added the tailnet host to `DEFAULT_ALLOWED_ORIGINS`; all 214 are pre-cutover uploads made when LPOS hardcoded `['app.leaderpass.com']`. No active re-lock flow exists (`leaderpass-ai` only reads CF), so a one-time backfill is durable. Backfilled all 214 videos via the CF API (additive — kept `app.leaderpass.com`, added the tailnet host), 214 OK / 0 fail. Post-scan: 0 ready videos still missing the tailnet origin (489 have both, 48 unrestricted, 1 with an extra origin). Also corrected a misleading code comment that claimed allowedOrigins does not gate HLS.

**Files changed:**
- `app/api/projects/[projectId]/media/[assetId]/frameio-stream/route.ts` — replaced the incorrect "allowedOrigins does NOT gate direct HLS — verified" comment with an accurate note explaining the CORS gating, the Firefox-vs-Safari difference, and the 2026-06-28 backfill.
- Cloudflare Stream account (external, not tracked): `allowedOrigins` updated on 214 videos.

**Implementation summary:** Read-only diagnosis first (CF API GET on the failing UID + full-account bucket scan by allowedOrigins), then an additive backfill script (`scratchpad/backfill_origins.py`) that, for every video whose `allowedOrigins` is non-empty and missing the tailnet host, POSTs `allowedOrigins = existing ∪ {lpos.tail856ed3.ts.net}`. Unrestricted (empty) videos were deliberately left untouched (they already accept all origins). Credentials sourced from Doppler (`CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_STREAM_TOKEN`).

**Decision rationale:** Backfill rather than a code change because the publish pipeline is already correct post-`555c779`; the broken videos are purely legacy state. Additive (union) rather than overwrite so platform hotlink protection (`app.leaderpass.com`) is preserved and no origin is ever removed. Left unrestricted videos alone since they are not affected.

**Alternatives considered:** (1) Proxying the HLS manifest + segments through the LPOS same-origin route to eliminate CORS entirely — the durable architectural fix, but heavier (manifest rewriting + segment proxying) and deferred. (2) Setting `allowedOrigins = *` — rejected, weakens hotlink protection on the public platform. (3) Fixing only C3.mp4 — rejected, 206 videos were affected.
...
## 2026-07-14T19:25:38Z — Delivery links: single checkbox "Edit videos" modal with add + remove

**User prompt (verbatim):** "I'm looking for something like this. [screenshot of Frame "Edit assets" modal] When clicked, it shows a simple list of all the videos with the ones already included checked and the ability to check new ones. I'd be on board to add the ability to remove from here, as well."

**Assistant response summary:** Replaced the delivery "Manage videos" modal's two-section (read-only list + separate add list) layout with a single Frame-style checkbox list of every project video — the ones already on the link start checked; check to add, uncheck to remove; "Save changes" commits the diff. Added the missing server-side remove path so unchecking actually deletes.

**Files changed:**
- `lpos-ingest/server.js` — new `DELETE /api/delivery/:token/assets/:id` (deletes the row + its R2 objects: original, proxy, thumbnail, and transcripts). Add endpoint (`POST .../assets`) now also dedups by **filename** (not just `asset_id`) so an untracked/legacy row can't be physically duplicated.
- `lpos-dashboard/app/api/projects/[projectId]/delivery/[token]/assets/[itemId]/route.ts` (new) — `DELETE` proxy to the ingest remove endpoint.
- `lpos-dashboard/components/projects/DeliveryPanel.tsx` — `ManageVideosModal` rewritten to one `sh-modal-asset-list` of project assets: included rows keyed `item:{id}` (start checked, carry a version badge), addable rows keyed `asset:{assetId}`, non-deliverable assets shown disabled ("no local file"), and orphan link items (on the link but no matching project asset) appended as checked/removable rows. Save diffs to add (POST) + remove (DELETE per item); "Refresh to latest (N)" kept in the list header when anything is stale. Client `sanitizeFilename` mirror added to match untracked items back to a project asset by filename.

**Implementation summary:** Membership is matched by `asset_id` first, then a filename fallback (client mirror of the server `sanitize`) so legacy/untracked delivery items still show as checked. Remove is immediate (DELETE); add is the existing background upload job. Server dedups adds by asset_id AND filename as a backstop against the filename fallback mis-detecting inclusion. Refresh gating unchanged (still asset_id + on-disk only). The two ingest DELETE routes (`/:token` vs `/:token/assets/:id`) don't collide — different segment counts.

**Decision rationale:** One checkbox list per the user's reference (Frame's Edit-assets modal) is the clearest add/remove affordance and reuses the exact `sh-modal-asset-*` classes. Filename dedup on the server makes the filename-fallback matching safe (worst case a rename mis-detects inclusion in the UI, but a duplicate upload is still prevented).

**Alternatives considered:** (a) Keep the two-section add/read-only layout — rejected, user explicitly wants the single checkbox list with remove. (b) Match untracked items by `asset_id` only (no filename fallback) — rejected; legacy links would show all-unchecked and risk duplicate adds. (c) Hard-delete via `asset_id` instead of row `id` — rejected; row `id` is unambiguous and also covers null-`asset_id` legacy/orphan rows.

**Commands / checks run:** `npx tsc --noEmit -p tsconfig.json` (0 errors); `node --check server.js` (OK). Dev server not started (user manages lifecycle); visual + end-to-end drive left to the user after ingest redeploy.

**Assumptions / follow-ups:** Requires the ingest redeploy for the remove endpoint + filename dedup. Removing a video deletes its R2 objects immediately — a client mid-download of that specific file would fail (acceptable; it's an intentional removal). Filename-fallback inclusion matching can mis-detect if a file was renamed between versions; low-stakes for the management list and backstopped by server dedup.

## 2026-07-15T13:51:53Z — Lighting: auto-poll now re-discovers fixtures that come online mid-session

**User prompt (verbatim):** "I'm having connection issues between fixture #1 in the white void set in lighting in lpos. It seems to be connected in the amaran app but not in LPOS."

**Response summary:** Diagnosed and fixed. A read-only WebSocket probe against Amaran Desktop confirmed Void fixture #1 (`400K5-F2DFEA`, "amaran 300c #1") was live in Amaran's `get_fixture_list` (19 fixtures reported), so this was not a pairing / nodeId-drift issue. Root cause was LPOS-side: `AmaranService.refreshStatus()` (the 30s auto-poll) only re-pulled state for already-known fixtures and re-ran `get_fixture_list` solely when the list was empty. Discovery otherwise ran only on connect or a manual Refresh, so any fixture that powered on / Bluetooth-reconnected after LPOS's last discovery stayed invisible in LPOS while Amaran showed it connected. Immediate operator workaround: gear → Refresh (rediscover). Durable fix: added `reconcileFixtureList()` and called it at the top of the poll's known-fixtures branch so newly-appeared fixtures self-heal within one poll cycle.

**Files changed:**
- `lib/services/amaran-service.ts` — added `reconcileFixtureList()` (additive merge of new fixtures from `get_fixture_list`, does NOT remove dropped ones); `refreshStatus()` now reconciles the list before pulling per-fixture state and logs any newly discovered nodeIds.

**Implementation summary:** `reconcileFixtureList()` fetches `get_fixture_list`, diffs against `_fixtures` by nodeId, and pushes any unknown fixtures (with `detectFixtureCapabilities`). `refreshStatus()` calls it (guarded by `.catch`) when `_fixtures.length > 0`, then pulls state for all fixtures including the new ones and emits status.

**Decision rationale:** Additive-only merge preserves the existing "discovered fixtures persist" behaviour — a momentary BT blip must not yank a live tile out from under an operator mid-shoot — while closing the gap that hid newly-connected lights. Reconciling inside the existing poll avoids adding a second timer and reuses the serial request queue (one extra `get_fixture_list` per 30s is negligible).

**Alternatives considered:** (1) Call `discoverAndRefresh()` on every poll — rejected: it replaces `_fixtures` wholesale, so a dropped light would vanish from the UI. (2) Leave as-is and rely on manual Refresh — rejected: recurring operator confusion; the poll should self-heal.
...
## 2026-07-23T16:43:08Z — Cloudflare library: correct storage budget cap to 4000 minutes

**User prompt (verbatim):** "Our cloudflare total minutes counter in admin is set to 3000 when our actual total is 4000. Is it just set to 3000 by default instead of reading the actual value?" → then: "Just set it to 4000 for now for ease and once you're done, commit and push"

**Assistant response summary:** Confirmed the admin "total minutes" denominator is a hardcoded UI constant, not a live/Cloudflare-read value. The used-minutes numerator IS real (summed from live Cloudflare video durations in `app/api/admin/cloudflare-videos/route.ts`), but the budget cap is the static `BUDGET_MINUTES` constant in `CloudflareLibraryPanel.tsx`. Per user request, bumped it from 3000 to 4000 to match the actual Stream plan.

**Files changed:**
- `components/settings/CloudflareLibraryPanel.tsx` — `BUDGET_MINUTES` constant 3000 → 4000.

**Implementation summary:** One-line constant change. Nothing queries Cloudflare's subscription/quota endpoint (Cloudflare's storage-usage API returns usage, not the plan cap), so the cap remains a manually-maintained constant.

**Decision rationale:** User explicitly chose the quick fix ("for now for ease") over wiring an admin Setting or live quota fetch. Flagged that this will drift again on the next plan change and that a SQLite-backed admin Setting is the more durable option — deferred by user choice.

**Alternatives considered:** (1) Move cap to admin Settings (SQLite) so it's editable without redeploy — the durable fix, deferred. (2) Fetch true quota live from Cloudflare — not cleanly available (storage-usage endpoint exposes usage, not plan cap).
...
## 2026-08-25 18:09 UTC — Download button for scripts (single + batch)

**User prompt:** "Can we quickly and simply add a download button to scripts (both singular and as a batch) following the format conventions set in other places in LPOS?"

**Response summary:** Added a download affordance to the project Scripts tab for both a single script and a multi-selection, reusing the existing transcripts download convention (1 file → raw file, 2+ → streamed .zip).

**Files changed:**
- `app/api/projects/[projectId]/scripts/download-zip/route.ts` (new) — POST `{ scriptIds }`; mirrors `transcripts/download-zip`.
- `components/projects/ScriptsTab.tsx` — `downloadScripts(ids)` client helper, context-menu "Download"/"Download N scripts" entries, per-row download button, `DownloadIcon`.

**Implementation summary:** Scripts are stored on disk as their original files (.docx/.pdf/.txt/.doc) with a per-project JSON registry. Single-file download reuses the pre-existing `GET scripts/[scriptId]/file` route (already sets `Content-Disposition: attachment`) via a browser-native direct `<a>` link. Batch download adds a new `scripts/download-zip` route that resolves each selected `scriptId` to its on-disk file, skips missing ones, and — following the established rule — returns the raw single file when exactly one resolves or a streamed `archiver` zip (`scripts-<project>.zip`, level 6) for two or more, with entries named by `originalFilename` and collision-deduped. Client `downloadScripts()` is a direct port of `batchDownload()` in `ProjectDetail.tsx`: direct link for one, `fetch`→blob→`Content-Disposition` filename→object-URL for many. UI: added a "Download" context-menu item (labels "Download N scripts" when multiple are selected, wired to the tab's existing `selectedIds` set) and an always-visible per-row download button. Download is offered even in `readOnly`/guest mode since it is non-destructive; delete stays gated on `!readOnly`.

**Decision rationale:** Reused the exact transcripts download pattern (server route + "no zip for one" + client blob-save helper) for consistency and minimal surface area rather than inventing a new mechanism. Shipped the raw stored files (not a re-rendered .docx from edited HTML) because that is the faithful, lossless artifact and matches how the single-file route already behaves.

**Alternatives considered:** (1) Server-side re-render to a uniform format (e.g. HTML→.docx via the content route's `html-to-docx`) — rejected; lossy for PDFs and unnecessary, users expect the file they uploaded. (2) A dedicated batch "action bar" like some other tabs — rejected as heavier than needed; the Scripts tab's interaction model is the context menu, so batch download lives there alongside batch delete.

**Commands/checks:** `npx tsc --noEmit` → exit 0 (clean). Dev server not run (user manages lifecycle); not exercised in-browser.

**Assumptions / follow-ups:** Single downloads deliver the original file as-is (unedited); edits made in the rich-text editor persist to the `.docx` regenerated by the content route, and that regenerated `.docx` is the stored file for docx originals, so those reflect edits — .pdf/.txt originals download unchanged. No new permission scope added.

---

## 2026-08-26 11:16 EDT — EditPanel Feedback / feature-request button synced to the shared Wish List

**User prompt (verbatim):** "Let's add a feature request/to-do button in edit panel that will sync with LPOS and update across all instances of editpanel so I can see what users are facing and asking for. Make sense?"

**Assistant response summary:** Added a "Feedback" button to EditPanel that lets editors file feature requests / to-dos, syncing to the existing LPOS shell **Wish List** (rather than a parallel store) so every EditPanel instance shows the same list with live Open/Done status and the LPOS owner reviews/manages them in the dashboard. Design confirmed with the user: reuse the Wish List with a source badge; editors submit + see all with status (management stays in LPOS).

**Files changed (lpos-dashboard, this commit):**
- `lib/models/wish.ts` — `WishSource` type + `source` / `sourceInstance` fields on `WishItem`.
- `lib/store/core-db.ts` — `wishes` table gains `source` (NOT NULL DEFAULT 'dashboard') + `source_instance` + `idx_wishes_source`, in `initSchema` and a new **v27** idempotent migration for existing DBs.
- `lib/store/wish-store.ts` — row mapping + `create()` accept/emit `source` / `sourceInstance` (defaults preserve dashboard behaviour).
- `app/api/ep/wishes/route.ts` (new) — EP-token GET (shared list) + POST (create); POST stamps `source='editpanel'`, attributes to the token's LPOS user, records the submitting machine from `instance`.
- `components/shell/WishListButton.tsx` — dashboard Wish List rows show an "EditPanel" badge + machine name for editpanel-origin wishes.
- `app/globals.css` — `.wishlist-badge` style.

**Files changed (editpanel repo, separate commit):**
- `electron/workers/lpos_client.js` — `listWishes()` / `createWish()`.
- `electron/main.js` — `lpos:wishes-list` / `lpos:wishes-create` IPC handlers (create injects display-name/hostname as `instance`).
- `electron/preload.js` — `window.lposAPI.listWishes` / `createWish`.
- `electron/renderer/components/FeedbackOverlay.jsx` (new) — modal: submit form + shared list, polls every 15s while open.
- `electron/renderer/App.jsx` — `feedbackOpen` state, status-bar "Feedback" button, overlay mount.
- `electron/renderer/index.html` — register FeedbackOverlay.jsx.
- `electron/renderer/styles.css` — feedback overlay/button styles.

**Implementation summary:** LPOS is the single source of truth; "sync across all instances" is achieved by each EditPanel polling `GET /api/ep/wishes` (15s while the panel is open) — no push channel needed. Submissions go through the existing `X-EP-Token` machine auth (`requireEpToken`), so editors are attributed to the LPOS user their machine is linked to, plus a machine label so the owner can tell who raised what. The `/api/ep/*` prefix is already exempt in `middleware.ts`, so no middleware change. Reused the shipped `wishes` table/store/UI instead of a new `feature_requests` table: one review surface, less code, and it's already covered by the nightly SQLite backup. Additive columns + v27 migration keep every pre-existing wish valid (defaults to source='dashboard').

**Decision rationale:** The dashboard shell already had a Wish List literally tooltip'd "request a feature"; feeding EditPanel requests into it (tagged by source) gives the owner a single place to see internal + editor requests and reuses proven CRUD/auth/backup paths. Editors get submit + read-only status (management deliberately stays in the dashboard) per the confirmed scope.

**Alternatives considered:** (1) Separate `feature_requests` table + dedicated LPOS admin page with open/in-progress/done — rejected as more surface area for no extra value once the Wish List already exists. (2) main-process poller broadcasting a `wishes-updated` event to the renderer — rejected in favour of simpler renderer-side polling scoped to when the panel is open.

**Commands/checks:** `npx tsc --noEmit` (lpos-dashboard) → exit 0. `node --check` on main.js/preload.js/lpos_client.js → OK. JSX not runtime-verified (dev server lifecycle is user-managed; no Babel toolchain locally to transform the in-browser `.jsx`).

**Assumptions / follow-ups:** Editor identity = the LPOS user linked to that EditPanel machine's token; multiple editors sharing one machine all attribute to that user (machine label disambiguates). No EP-side edit/delete/complete (by design). Possible follow-ups if wanted: let editors delete their own accidental submission; add an in-progress status; badge dashboard-origin rows inside EditPanel.
