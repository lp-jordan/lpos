...
- **Duotone source images (done):** a duotone tile can carry a real source image — **uploaded** per tile, or pulled from its **linked video's poster frame** (fetched server-side). Images are stored on **local disk** (`{DATA_DIR}/platform/tile-images/{tileId}`, mime tracked in `platform_tiles.image_mime`) and served same-origin at `/api/platform/tiles/:id/image` — so the browser can composite + export them without tainting the canvas, and the Cloudflare video poster is never touched. The engine composites the image under the brand duotone filter (`buildTileBackgroundSVG` `imageHref` opt: a same-origin URL for the live preview, a base64 data-URI for Export since SVG-as-image blocks external refs). No image → the procedural stand-in. Currently per-tile; a pass-level bulk-assign is a later convenience.
- **Generated hero images (pipework done; API not wired):** a duotone/geometric tile's source image can also be **generated** from its title + description. `POST /api/platform/tiles/:id/generate-image` builds a prompt (`lib/platform/image-style.ts` → subject from the tile's words + the house `STYLE_DIRECTIVE` block read live from `docs/platform-image-style.md`), calls the single generation seam (`lib/platform/image-generate.ts`), and stores the bytes in the **same local seam** as uploaded images — so it flows through the existing duotone renderer + Export unchanged. Provenance + prompt are tracked (`platform_tiles.image_source` ∈ `uploaded`/`poster`/`generated`, `image_prompt`). The inspector's image control is now **Generate / Upload / Use video frame** with an "Edit prompt" override and Regenerate. **The seam currently returns a fixed placeholder** (a tonal SVG that duotones like a real hero); going live = implement the commented OpenAI `gpt-image-1` branch in `image-generate.ts` + set `OPENAI_API_KEY` in Doppler (needs OpenAI org verification). Design art still never touches the Cloudflare poster. ⚠️ `{DATA_DIR}/platform/tile-images/` is still not covered by the nightly BackupService — grows more valuable once generation is live.
