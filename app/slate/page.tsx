...
export default function SlatePage() {
  ...
  function handleStudioTabChange(tab: typeof studioTab) {
    setStudioTab(tab);
        ...
        <nav className="sl-pill-nav" aria-label="Studio tabs">
          {([
            ...
            <button
              key={tab.id}
              type="button"
              className={`sl-pill${studioTab === tab.id ? ' sl-pill--active' : ''}${tab.soon ? ' sl-pill--soon' : ''}`}
              onClick={() => !tab.soon && handleStudioTabChange(tab.id as typeof studioTab)}
