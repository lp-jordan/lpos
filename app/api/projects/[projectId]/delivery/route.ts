...
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  const res = await fetch(
    ...
    { headers: { 'x-api-key': INGEST_API_KEY }, signal: req.signal },
  )
  if (!res.ok) {
...
export async function POST(req: NextRequest, { params }: Ctx) {
  ...
  void (async () => {
    try {
      const r2Assets: { r2_key: string; filename: string; file_size: number; mime_type: string }[] = []
      ...
      for (let i = 0; i < total; i++) {
        ...
        r2Assets.push({ r2_key: r2Key, filename, file_size: fileSize, mime_type: mimeType })
