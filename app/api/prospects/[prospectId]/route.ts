...
export async function PATCH(req: NextRequest, { params }: Ctx) {
  ...
  if (typeof body.company  === 'string') {
    ...
    // Keep the clients table in sync when renaming an active client
    if (before.status === 'active') {
      getClientStore().upsertForProspect(prospectId, body.company, session!.userId);
    }
  }
...
export async function DELETE(req: NextRequest, { params }: Ctx) {
  ...
  // Remove the linked clients entry so Projects no longer surfaces this client
  getClientStore().deleteByProspectId(prospectId);
