...
export function EpReleasesCard() {
  ...
  async function save() {
    ...
    setError(null);
    try {
      const res = await fetch('/api/ep-updates/config', {
        method:  'POST',
  ...
  return (
    <div className="storage-settings-card">
      <div>
        <h2 className="storage-settings-section-title">EditPanel Updates</h2>
        <p className="storage-settings-muted">
          Point LPOS at the folder where the Windows <code>electron-builder</code> build writes its
          output (the shared NAS folder). Run a build on the Windows machine and LPOS will automatically
          serve the new version to editpanel users via the <code>/ep-update</code> page.
        </p>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginTop: '1rem' }}>
        <label style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)' }}>
          Build output directory (NAS, Mac mount)
          <input
            type="text"
            value={watchDir}
            onChange={(e) => setWatchDir(e.target.value)}
            placeholder="/Volumes/LeaderPass Main/LPOS/editpanel dist"
            style={{
              display:     'block',
              width:       '100%',
              marginTop:   '0.4rem',
