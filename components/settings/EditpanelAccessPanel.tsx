...
export function EditpanelAccessPanel() {
  ...
  async function load() {
    try {
      ...
      const [usersRes, accessRes] = await Promise.all([
        fetch('/api/users'),
        fetch('/api/admin/editpanel-access'),
      ]);
      ...
      setError('Could not load EditPanel access list. Please try again.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleGrant() {
    ...
    setActionError(null);
    try {
      const res  = await fetch('/api/admin/editpanel-access', {
        method:  'POST',
  ...
  return (
    <div className="storage-settings-card">
      <div>
        <h2 className="storage-settings-section-title">EditPanel Access</h2>
        <p className="storage-settings-muted">
          Users listed here see a small EditPanel icon on the home screen that links to the
          download page (<code>/ep-update</code>). Admins always see it. This only controls the
          shortcut&apos;s visibility — the download page itself stays reachable to anyone with the link.
        </p>
      </div>
